...
// indicates that the blossom in the ith bed is blooming, and plots[i] == 0 means 
// it is dormant. You are also given an integer n which represents the number of 
// days this transformation ritual takes place.
...
// Explanation: After one billion days of transformation, the garden settles into 
// the final state [0, 0, 1, 1, 1, 1, 1, 0].
...
import java.util.*;
class Solution{
    private static int[] solve(int[] plots){
        int l=0,r=2;
        int[] res= new int[8];
        while(r<8){
            if(plots[l]==plots[r]) res[l+1]=1;
            else res[l+1]=0;
            l++;
            r++;
        }
    ...
    public static void main(String[] args){
        ...
        int[] plots = new int[8];
        for(int i=0;i<8;i++) plots[i]=sc.nextInt();
        int days=sc.nextInt();
        if(days==0){
