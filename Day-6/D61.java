// In a f1 ilm festival, there is a lineup of movies, each with a rating. The festival 
...
class Solution{
    public static void main(String[] args){
        ...
        int k=sc.nextInt();
        int m=sc.nextInt();
        int[] movieRating = new int[n];
        for(int i=0;i<n;i++) movieRating[i]=sc.nextInt();
        ...
        while(r<n){
            sum+=movieRating[r];
            if(restricted.contains(movieRating[r])){
                l=r+1;
                r++;
                sum=0;
                track = new HashSet<>();
            ...
            else if(track.contains(movieRating[r])){
                l=r;
                track=new HashSet<>();
                track.add(movieRating[r]);
                sum=movieRating[r];
                r++;
                continue;
            }
